...
import { BASE_CREATED, BASE_UPDATED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.SettingAPI.BaseAPI.UpdateAPI({
      id,
			data: {
    ...
    .then(response => {
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(BASE_UPDATED, serverResponse.update);	
        navigate(FinalRouteName.ContentRoute.SidebarRoute.SettingRoute.BaseRoute.ListRoute)
      }
    })
    .catch(error => {
        // console.log(error.response.data);
