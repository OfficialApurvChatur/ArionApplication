...
import { ADMIN_HERO_UPDATED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.SettingAPI.AdminHeroAPI.UpdateAPI({
      id,
			data: {
    ...
    .then(response => {
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(ADMIN_HERO_UPDATED, serverResponse.update);	
