...
const BlogCardDeletePage = ({ ReduxUltimate }) => {
	// Variables
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.BlogCardDeletePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Blog Card')[0]?.access?.delete ? 
...
export default BlogCardDeletePage