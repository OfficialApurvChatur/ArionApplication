...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.PortfolioCardAPI.RetrieveAPI({id})
    .then(response => {
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast) => {
    ...
    formData.append('dLinks', JSON.stringify(Redux.state.FormObject.FormValue?.links || ""));
    formData.append('dReferences', JSON.stringify(Redux.state.FormObject.FormValue?.references || ""));
    formData.append('dPortfolioImages', JSON.stringify(Redux.state.FormObject.FormValue?.portfolioImages
      ?.map(each => {
        if (each?.public_id) return each
      })
      ?.filter(each => each) || ""
    ));
    Redux.state.FormObject.FormValue?.portfolioImages?.length && 
      Redux.state.FormObject.FormValue?.portfolioImages?.forEach(file => {
        if (file.name) formData.append('dPortfolioImages', file);
      });

    API.ContentAPI.SidebarAPI.MainAPI.PortfolioCardAPI.UpdateAPI({
      id,
			data: formData
    })
    .then(response => {
      ...
      if (serverResponse.success === true) {
        ...
        navigate(FinalRouteName.ContentRoute.SidebarRoute.MainRoute.PortfolioCardRoute.ListRoute)
      }
    })
    .catch(error => {
        // console.log(error.response.data);
