...
const SubBranchUpdatePage = ({ ReduxUltimate }) => {
	// Variables
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.SubBranchUpdatePageState),
  ...
  // All Render
	...
	// Second Render
	useEffect(() => {
		Redux.state.RequiredObject?.SubSubBranchList && APICalls.RetrieveAPICall()
	}, [Redux.state.RequiredObject?.SubSubBranchList])
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Branch Group')[0]?.access?.update ? 
...
export default SubBranchUpdatePage