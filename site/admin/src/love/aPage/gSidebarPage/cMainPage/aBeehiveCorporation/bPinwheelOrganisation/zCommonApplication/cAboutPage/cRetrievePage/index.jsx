...
const AboutRetrievePage = ({ ReduxUltimate }) => {
	// Variables
	const { id } = useParams();

  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.AboutRetrievePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'About')[0]?.access?.retrieve ? 
...
export default AboutRetrievePage