...
import { BRANCH_SECTION_UPDATED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.BranchAPI.RetrieveAPI({id})
    .then(response => {
      ...
      if (serverResponse.success === true) {
        Redux.dispatch({ type: Redux.action.FormObject, payload: {
          ...
          FormValue: {
            ...
            subBranches: serverResponse.retrieve.cSubBranches,
          }
        }})
      }
    })
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.BranchAPI.UpdateAPI({
      id,
			data: {
        ...
        cSubBranches: Redux.state.FormObject.FormValue.subBranches,
			}
    })
    .then(response => {
      // console.log(response.data);
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(BRANCH_SECTION_UPDATED, serverResponse.update);	
        navigate(FinalRouteName.ContentRoute.SidebarRoute.MainRoute.BranchRoute.ListRoute)
      }
    })
    .catch(error => {
        // console.log(error.response.data);
  ...
  // Other APIs
  // Sub Branch List API
  SubBranchListAPI: (Redux, ReduxUltimate) => {
    loading(ReduxUltimate, true)

    API.ContentAPI.SidebarAPI.MainAPI.SubBranchAPI.ListAPI()
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
      ...
      if (serverResponse.success === true) {
        Redux.dispatch({ type: Redux.action.RequiredObject, payload: {
          ...
          SubBranchList: serverResponse.list.map(each => {
            return {
              id: each._id,
              title: each.aTitle,
            }
