...
import { PROJECT_GROUP_UPDATED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.SubBranchAPI.RetrieveAPI({id})
    .then(response => {
      ...
      if (serverResponse.success === true) {
        Redux.dispatch({ type: Redux.action.FormObject, payload: {
          ...
          FormValue: {
            ...
            subSubBranches: serverResponse.retrieve.cSubSubBranches,
          }
        }})
      }
    })
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.SubBranchAPI.UpdateAPI({
      id,
			data: {
    ...
    .then(response => {
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(PROJECT_GROUP_UPDATED, serverResponse.update);	
