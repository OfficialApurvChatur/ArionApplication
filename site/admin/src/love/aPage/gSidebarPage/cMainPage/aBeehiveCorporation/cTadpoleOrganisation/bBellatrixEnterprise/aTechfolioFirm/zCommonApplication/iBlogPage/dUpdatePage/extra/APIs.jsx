...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.BlogAPI.RetrieveAPI({id})
    .then(response => {
      ...
      if (serverResponse.success === true) {
        Redux.dispatch({ type: Redux.action.FormObject, payload: {
          ...
          FormValue: {
            ...
            blogCards: serverResponse.retrieve.cBlogCards,
          }
        }})
      }
    })
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.BlogAPI.UpdateAPI({
      id,
			data: {
        ...
        cBlogCards: Redux.state.FormObject.FormValue.blogCards,
			}
    })
    .then(response => {
      // console.log(response.data);
      ...
      if (serverResponse.success === true) {
        ...
        navigate(FinalRouteName.ContentRoute.SidebarRoute.MainRoute.BlogRoute.ListRoute)
      }
    })
    .catch(error => {
        // console.log(error.response.data);
  ...
  // Other APIs
  // Blog Card List API
  BlogCardListAPI: (Redux, ReduxUltimate) => {
    loading(ReduxUltimate, true)

    API.ContentAPI.SidebarAPI.MainAPI.BlogCardAPI.ListAPI()
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
