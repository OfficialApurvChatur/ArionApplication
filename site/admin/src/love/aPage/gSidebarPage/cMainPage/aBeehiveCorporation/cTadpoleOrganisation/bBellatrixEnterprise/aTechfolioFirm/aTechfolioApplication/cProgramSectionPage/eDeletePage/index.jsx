...
const ProgramSectionDeletePage = ({ ReduxUltimate }) => {
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.ProgramSectionDeletePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Program Section')[0]?.access?.delete ? 
...
export default ProgramSectionDeletePage