...
const ProjectSectionUpdatePage = ({ ReduxUltimate }) => {
	// Variables
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.ProjectSectionUpdatePageState),
  ...
  // All Render
	...
	// Second Render
	useEffect(() => {
		Redux.state.RequiredObject?.ProjectGroupList && APICalls.RetrieveAPICall()
	}, [Redux.state.RequiredObject?.ProjectGroupList])
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Project Section')[0]?.access?.update ? 
...
export default ProjectSectionUpdatePage