...
import { BRANCH_DELETED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.SubSubBranchAPI.RetrieveAPI({id})
    .then(response => {
   ...
   DeleteAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.SubSubBranchAPI.DeleteAPI({id})
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(BRANCH_DELETED, serverResponse.delete);	
        navigate(FinalRouteName.ContentRoute.SidebarRoute.MainRoute.SubSubBranchRoute.ListRoute)
      }
    })
    .catch(error => {
        // console.log(error.response.data);
