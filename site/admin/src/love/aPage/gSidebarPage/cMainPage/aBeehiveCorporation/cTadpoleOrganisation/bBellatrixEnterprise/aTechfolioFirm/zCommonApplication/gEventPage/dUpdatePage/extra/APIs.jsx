...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.EventAPI.RetrieveAPI({id})
    .then(response => {
      ...
      if (serverResponse.success === true) {
        Redux.dispatch({ type: Redux.action.FormObject, payload: {
          ...
          FormValue: {
            ...
            eventCards: serverResponse.retrieve.cEventCards,
          }
        }})
      }
    })
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.EventAPI.UpdateAPI({
      id,
			data: {
        ...
        cEventCards: Redux.state.FormObject.FormValue.eventCards,
			}
    })
    .then(response => {
      // console.log(response.data);
      ...
      if (serverResponse.success === true) {
        ...
        navigate(FinalRouteName.ContentRoute.SidebarRoute.MainRoute.EventRoute.ListRoute)
      }
    })
    .catch(error => {
        // console.log(error.response.data);
  ...
  // Other APIs
  // Event Card List API
  EventCardListAPI: (Redux, ReduxUltimate) => {
    loading(ReduxUltimate, true)

    API.ContentAPI.SidebarAPI.MainAPI.EventCardAPI.ListAPI()
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
