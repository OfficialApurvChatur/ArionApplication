...
const PortfolioDeletePage = ({ ReduxUltimate }) => {
	// Variables
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.PortfolioDeletePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Portfolio')[0]?.access?.delete ? 
...
export default PortfolioDeletePage