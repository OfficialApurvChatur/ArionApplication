...
const HeroRetrievePage = ({ ReduxUltimate }) => {
	// Variables
	const { id } = useParams();

  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.HeroRetrievePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Hero')[0]?.access?.retrieve ? 
