...
const ExperienceDeletePage = ({ ReduxUltimate }) => {
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.ExperienceDeletePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Experience')[0]?.access?.delete ? 
...
export default ExperienceDeletePage