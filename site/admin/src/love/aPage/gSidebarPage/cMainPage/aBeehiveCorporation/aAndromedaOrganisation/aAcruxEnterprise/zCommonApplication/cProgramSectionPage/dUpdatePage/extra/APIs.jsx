...
import { PROGRAM_SECTION_UPDATED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.ProgramSectionAPI.RetrieveAPI({id})
    .then(response => {
      ...
      if (serverResponse.success === true) {
        Redux.dispatch({ type: Redux.action.FormObject, payload: {
          ...
          FormValue: {
            ...
            programs: serverResponse.retrieve.cPrograms,
          }
        }})
      }
    })
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.MainAPI.ProgramSectionAPI.UpdateAPI({
      id,
			data: {
    ...
    .then(response => {
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(PROGRAM_SECTION_UPDATED, serverResponse.update);	
