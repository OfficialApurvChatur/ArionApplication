...
const ExperienceRetrievePage = ({ ReduxUltimate }) => {
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.ExperienceRetrievePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Experience')[0]?.access?.retrieve ? 
...
export default ExperienceRetrievePage