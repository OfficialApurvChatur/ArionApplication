...
const BlogRetrievePage = ({ ReduxUltimate }) => {
	// Variables
	const { id } = useParams();

  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.BlogRetrievePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Blog')[0]?.access?.retrieve ? 
...
export default BlogRetrievePage