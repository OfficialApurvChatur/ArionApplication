...
const ProjectGroupUpdatePage = ({ ReduxUltimate }) => {
	// Variables
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.ProjectGroupUpdatePageState),
  ...
  // All Render
	...
	// Second Render
	useEffect(() => {
		Redux.state.RequiredObject?.ProjectList && APICalls.RetrieveAPICall()
	}, [Redux.state.RequiredObject?.ProjectList])
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Project Group')[0]?.access?.update ? 
...
export default ProjectGroupUpdatePage