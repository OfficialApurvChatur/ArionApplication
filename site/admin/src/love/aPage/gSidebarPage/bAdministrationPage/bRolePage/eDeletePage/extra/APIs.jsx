...
import { ROLE_DELETED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
   ...
   DeleteAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.AdministrationAPI.RoleAPI.DeleteAPI({id})
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(ROLE_DELETED, serverResponse.delete);	
