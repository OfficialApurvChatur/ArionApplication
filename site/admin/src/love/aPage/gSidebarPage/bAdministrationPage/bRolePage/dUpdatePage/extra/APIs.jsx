...
import { ROLE_UPDATED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
    ...
    API.ContentAPI.SidebarAPI.AdministrationAPI.RoleAPI.RetrieveAPI({id})
    .then(response => {
      ...
      if (serverResponse.success === true) {
        Redux.dispatch({ type: Redux.action.FormObject, payload: {
          ...
          FormValue: {
            ...
            menus: serverResponse.retrieve.cMenus,
          }
        }})
      }
    })
   ...
   UpdateAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.AdministrationAPI.RoleAPI.UpdateAPI({
      id,
			data: {
        ...
        cMenus: Redux.state.FormObject.FormValue.menus
			}
    })
    .then(response => {
      // console.log(response.data);
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(ROLE_UPDATED, serverResponse.update);	
        navigate(FinalRouteName.ContentRoute.SidebarRoute.AdministrationRoute.RoleRoute.ListRoute)
      }
    })
    .catch(error => {
        // console.log(error.response.data);
  ...
  // Other APIs
  MenuListAPI: (Redux, ReduxUltimate) => {
    loading(ReduxUltimate, true)

    API.ContentAPI.SidebarAPI.AdministrationAPI.MenuAPI.ListAPI()
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
