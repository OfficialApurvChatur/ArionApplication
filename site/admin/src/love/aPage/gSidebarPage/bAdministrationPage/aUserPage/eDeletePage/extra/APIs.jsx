...
import { USER_DELETED } from "@/love/iSocket/event";
...
const APIs = {
  ...
  RetrieveAPI: (Redux, ReduxUltimate, id) => {
   ...
   DeleteAPI: (Redux, ReduxUltimate, id, navigate, toast, socket) => {
    ...
    API.ContentAPI.SidebarAPI.AdministrationAPI.UserAPI.DeleteAPI({id})
    .then(response => {
      // console.log(response.data);
      const serverResponse = response.data;
      ...
      if (serverResponse.success === true) {
        ...
        socket.emit(USER_DELETED, serverResponse.delete);	
