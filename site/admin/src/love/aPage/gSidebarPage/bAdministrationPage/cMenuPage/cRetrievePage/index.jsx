...
const MenuRetrievePage = ({ ReduxUltimate }) => {
	// Variables
	const { id } = useParams();

  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.MenuRetrievePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Menu')[0]?.access?.retrieve ? 
