...
const MenuCreatePage = ({ ReduxUltimate }) => {
	// Variables
  ...
  // Redux
	const Redux = {
		state: useSelector((fullState) => fullState.MenuCreatePageState),
  ...
  return (
    <React.Fragment>
			{ReduxUltimate.state.ReceivedObject?.ProfileRetrieve?.cRole?.cMenus?.filter(each => each?.menu?.aTitle === 'Menu')[0]?.access?.create ? 
